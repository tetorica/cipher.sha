...
class SHA1 {
  ...
  SHA1State sha1Input(Uint8List messageArray, int length) {
    ...
    //
    //
    //this.messageBlock = messageArray;// & 0xFF;

    int i = this.messageBlockIndex;
    for (i = 0; i + 64 < length; i += 64) {
      //this.messageBlockIndex++;
      this.messageBlock.setRange(
          0, 64, messageArray, i); // = messageArray.sublist(i,i+64);// & 0xFF;
      this.messageBlockIndex = 64;

      if (this.lengthLow == 0xFFFFFE00) {
        this.lengthLow = 0;
        this.lengthHigh++;
      } else {
        this.lengthLow += 8 * 64;
    ...
    if (i < length) {
      this.messageBlock.setRange(0, length - i, messageArray, i); // & 0xFF;
      this.lengthLow += 8 * (length - i);
      this.messageBlockIndex += (length - i);
      if (this.messageBlockIndex == 64) {
        sha1ProccessMessageBlock();
      }
    }
  ...
  static int sha1CirclularShift(int x, int n) =>
